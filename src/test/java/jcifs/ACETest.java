...
class ACETest {
    ...
    class ConstantValidationTests {
        ...
        @ParameterizedTest
        @ValueSource(ints = {
            ACE.FILE_READ_DATA, ACE.FILE_WRITE_DATA, ACE.FILE_APPEND_DATA,
            ACE.FILE_READ_EA, ACE.FILE_WRITE_EA, ACE.FILE_EXECUTE,
            ACE.FILE_DELETE, ACE.FILE_READ_ATTRIBUTES, ACE.FILE_WRITE_ATTRIBUTES,
            ACE.DELETE, ACE.READ_CONTROL, ACE.WRITE_DAC, ACE.WRITE_OWNER, ACE.SYNCHRONIZE
        })
        ...
        void shouldBeValidPowersOfTwo(int constant) {
            ...
            assertEquals(0, constant & (constant - 1), 
                "Constant should be power of 2: " + Integer.toHexString(constant));
        }
        
        @ParameterizedTest
        @ValueSource(ints = {
            ACE.FLAGS_OBJECT_INHERIT, ACE.FLAGS_CONTAINER_INHERIT,
            ACE.FLAGS_NO_PROPAGATE, ACE.FLAGS_INHERIT_ONLY, ACE.FLAGS_INHERITED
        })
        ...
        void shouldBeValidFlagPowersOfTwo(int flag) {
            ...
            assertEquals(0, flag & (flag - 1), 
                "Flag should be power of 2: " + Integer.toHexString(flag));
        }
        ...
        void shouldHaveUniqueConstants() {
            int[] accessConstants = {
                ...
                ACE.FILE_DELETE, ACE.FILE_READ_ATTRIBUTES, ACE.FILE_WRITE_ATTRIBUTES,
                ACE.DELETE, ACE.READ_CONTROL, ACE.WRITE_DAC, ACE.WRITE_OWNER, ACE.SYNCHRONIZE,
                ACE.GENERIC_ALL, ACE.GENERIC_EXECUTE, ACE.GENERIC_WRITE, ACE.GENERIC_READ
            };
            ...
            for (int i = 0; i < accessConstants.length; i++) {
                for (int j = i + 1; j < accessConstants.length; j++) {
                    assertNotEquals(accessConstants[i], accessConstants[j], 
                        "Access constants should be unique: " + Integer.toHexString(accessConstants[i]) + 
                        " vs " + Integer.toHexString(accessConstants[j]));
                }
            }
            ...
            for (int i = 0; i < flagConstants.length; i++) {
                for (int j = i + 1; j < flagConstants.length; j++) {
                    assertNotEquals(flagConstants[i], flagConstants[j], 
                        "Flag constants should be unique: " + Integer.toHexString(flagConstants[i]) + 
                        " vs " + Integer.toHexString(flagConstants[j]));
                }
            }
        }
        ...
        void shouldHaveCorrectBitPositions() {
            ...
            assertEquals(0, Integer.numberOfTrailingZeros(ACE.FILE_READ_DATA), 
                "FILE_READ_DATA should be at bit position 0");
            assertEquals(1, Integer.numberOfTrailingZeros(ACE.FILE_WRITE_DATA), 
                "FILE_WRITE_DATA should be at bit position 1");
            assertEquals(28, Integer.numberOfTrailingZeros(ACE.GENERIC_ALL), 
                "GENERIC_ALL should be at bit position 28");
            assertEquals(31, Integer.numberOfTrailingZeros(ACE.GENERIC_READ), 
                "GENERIC_READ should be at bit position 31");
        }
    }
    ...
    class WindowsAccessCheckTests {
        ...
        void shouldSimulateDocumentationExample() {
            ...
            assertTrue((aliceDirectACE & ACE.FILE_READ_DATA) != 0, 
                "Alice's direct ACE should allow FILE_READ_DATA");
            assertFalse((aliceDirectACE & ACE.FILE_WRITE_DATA) != 0, 
                "Alice's direct ACE should not allow FILE_WRITE_DATA");
            
            // Inherited ACE: Allow Administrators 0x001F01FF
            int adminInheritedACE = 0x001F01FF;
            assertTrue((adminInheritedACE & ACE.FILE_READ_DATA) != 0, 
                "Admin inherited ACE should allow FILE_READ_DATA");
            assertTrue((adminInheritedACE & ACE.FILE_WRITE_DATA) != 0, 
                "Admin inherited ACE should allow FILE_WRITE_DATA");
        }
    ...
    class BoundaryConditionsTests {
        ...
        void shouldValidateConstantBitRanges() {
            ...
            assertTrue(ACE.DELETE >= 0x00010000 && ACE.DELETE <= 0x00FF0000, 
                "DELETE should be in standard rights range");
            assertTrue(ACE.READ_CONTROL >= 0x00010000 && ACE.READ_CONTROL <= 0x00FF0000, 
                "READ_CONTROL should be in standard rights range");
            assertTrue(ACE.SYNCHRONIZE >= 0x00010000 && ACE.SYNCHRONIZE <= 0x00FF0000, 
                "SYNCHRONIZE should be in standard rights range");
        ...
        void shouldHandleAccessMaskBoundaries() {
            ...
            int maxAccess = ACE.GENERIC_READ | ACE.GENERIC_WRITE | ACE.GENERIC_EXECUTE | ACE.GENERIC_ALL |
                           ACE.SYNCHRONIZE | ACE.WRITE_OWNER | ACE.WRITE_DAC | ACE.READ_CONTROL | ACE.DELETE |
                           ACE.FILE_WRITE_ATTRIBUTES | ACE.FILE_READ_ATTRIBUTES | ACE.FILE_DELETE |
                           ACE.FILE_EXECUTE | ACE.FILE_WRITE_EA | ACE.FILE_READ_EA |
                           ACE.FILE_APPEND_DATA | ACE.FILE_WRITE_DATA | ACE.FILE_READ_DATA;
    ...
    class InterfaceEdgeCaseTests {
        ...
        @Test
        @DisplayName("Should handle null SID from interface methods")
    ...
    class RealWorldUsageTests {
        ...
        void shouldHandleFileSystemPermissions() {
            ...
            int modifyMask = ACE.FILE_READ_DATA | ACE.FILE_WRITE_DATA | ACE.FILE_APPEND_DATA | 
                            ACE.FILE_DELETE | ACE.DELETE | ACE.FILE_READ_ATTRIBUTES | ACE.FILE_WRITE_ATTRIBUTES;
