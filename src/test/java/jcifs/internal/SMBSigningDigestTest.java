...
class SMBSigningDigestTest {
    ...
    void testSignWithDifferentOffsets() {
        ...
        verify(signingDigest, times(offsets.length)).sign(any(byte[].class), anyInt(), anyInt(), 
    ...
    void testMultipleSignOperations() {
        ...
        // Assert
        verify(signingDigest, times(iterations)).sign(any(byte[].class), anyInt(), anyInt(),
                                                       any(CommonServerMessageBlock.class),
                                                       any(CommonServerMessageBlock.class));
    }
    ...
    void testMultipleVerifyOperations() {
        // Arrange
        when(signingDigest.verify(any(byte[].class), anyInt(), anyInt(), anyInt(), any()))
    ...
    void testSignMethodArgumentCapture() {
        ...
        // Assert
        verify(signingDigest).sign(dataCaptor.capture(), offsetCaptor.capture(), 
                                   lengthCaptor.capture(), eq(request), eq(response));
    ...
    void testVerifyMethodArgumentCapture() {
        ...
        // Assert
        verify(signingDigest).verify(dataCaptor.capture(), offsetCaptor.capture(), 
                                     lengthCaptor.capture(), extraPadCaptor.capture(), eq(message));
