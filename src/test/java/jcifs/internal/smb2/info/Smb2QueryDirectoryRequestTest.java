...
class Smb2QueryDirectoryRequestTest {
    ...
    void testSetFileInformationClass() {
        ...
        assertEquals(Smb2QueryDirectoryRequest.FILE_FULL_DIRECTORY_INFO, 
                    buffer[Smb2Constants.SMB2_HEADER_LENGTH + 2]);
    }
    ...
    void testSetQueryFlags() {
        request = new Smb2QueryDirectoryRequest(mockConfig);
        byte flags = Smb2QueryDirectoryRequest.SMB2_RESTART_SCANS | 
                    Smb2QueryDirectoryRequest.SMB2_RETURN_SINGLE_ENTRY;
    ...
    void testSetFileIndex() {
        ...
        int readIndex = (buffer[Smb2Constants.SMB2_HEADER_LENGTH + 4] & 0xFF) |
                       ((buffer[Smb2Constants.SMB2_HEADER_LENGTH + 5] & 0xFF) << 8) |
    ...
    void testWriteBytesWireFormatComplete() {
        ...
        assertEquals(33, buffer[Smb2Constants.SMB2_HEADER_LENGTH] | 
                        (buffer[Smb2Constants.SMB2_HEADER_LENGTH + 1] << 8));
        
        // Verify file information class
        assertEquals(Smb2QueryDirectoryRequest.FILE_ID_FULL_DIRECTORY_INFO, 
                    buffer[Smb2Constants.SMB2_HEADER_LENGTH + 2]);
        
        // Verify query flags
        assertEquals(Smb2QueryDirectoryRequest.SMB2_INDEX_SPECIFIED, 
                    buffer[Smb2Constants.SMB2_HEADER_LENGTH + 3]);
        
        // Verify file index
        int readFileIndex = (buffer[Smb2Constants.SMB2_HEADER_LENGTH + 4] & 0xFF) |
                           ((buffer[Smb2Constants.SMB2_HEADER_LENGTH + 5] & 0xFF) << 8) |
                           ((buffer[Smb2Constants.SMB2_HEADER_LENGTH + 6] & 0xFF) << 16) |
                           ((buffer[Smb2Constants.SMB2_HEADER_LENGTH + 7] & 0xFF) << 24);
    ...
    void testWriteBytesWireFormatNoFileName() {
        ...
        int fnOffset = buffer[Smb2Constants.SMB2_HEADER_LENGTH + 24] | 
                      (buffer[Smb2Constants.SMB2_HEADER_LENGTH + 25] << 8);
        int fnLength = buffer[Smb2Constants.SMB2_HEADER_LENGTH + 26] | 
                      (buffer[Smb2Constants.SMB2_HEADER_LENGTH + 27] << 8);
    ...
    void testCombinedQueryFlags() {
        ...
        byte combinedFlags = (byte)(Smb2QueryDirectoryRequest.SMB2_RESTART_SCANS | 
                                    Smb2QueryDirectoryRequest.SMB2_RETURN_SINGLE_ENTRY |
                                    Smb2QueryDirectoryRequest.SMB2_INDEX_SPECIFIED |
                                    Smb2QueryDirectoryRequest.SMB2_REOPEN);
