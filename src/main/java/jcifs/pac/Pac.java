...
public class Pac {
    ...
    public Pac(byte[] data, Map<Integer, KerberosKey> keys) throws PACDecodingException {
                ...
                case PacConstants.SERVER_CHECKSUM:
                    ...
                    if (this.serverSignature == null) {
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug(String.format("Server signature is type %d @ %d len %d", this.serverSignature.getType(), bufferOffset,
                                    bufferSize));
                        }
        ...
        if (!MessageDigest.isEqual(this.serverSignature.getChecksum(), checksum)) {
            if (log.isDebugEnabled()) {
                log.debug(String.format("PAC signature validation failed, have: %s expected: %s type: %d len: %d",
