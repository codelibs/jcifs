...
import org.codelibs.jcifs.smb1.DfsReferral;
...
import org.codelibs.jcifs.smb1.util.MimeMap;
...
public class NetworkExplorer extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        Config.setProperty("jcifs.client.soTimeout", "600000");
        Config.setProperty("jcifs.client.attrExpirationPeriod", "300000");
        ...
        if (Config.getProperty("jcifs.client.username") == null) {
            ntlmSsp = new NtlmSsp();
        } else {
        ...
        try {
            mimeMap = new MimeMap();
            is = getClass().getClassLoader().getResourceAsStream("org/codelibs/jcifs/smb1/http/ne.css");
    ...
    protected void doDirectory(final HttpServletRequest req, final HttpServletResponse resp, final SmbFile dir) throws IOException {
        ...
        for (i = 0; i < dirents.length; i++) {
            ...
            for (j = 0; iter.hasNext(); j++) {
                ...
                } else if (sort == 3 && compareDates(dirents[i], name, (SmbFile) iter.next()) < 0) {
                    break;
                }
            }
