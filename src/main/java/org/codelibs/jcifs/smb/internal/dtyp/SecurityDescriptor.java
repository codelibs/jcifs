...
public class SecurityDescriptor implements SecurityInfo {
     ...
     */
    private ACE[] aces;
    private SIDObject ownerUserSid, ownerGroupSid;
    ...
    /**
     * Gets the owner group SIDObject of this security descriptor.
     *
     * @return the security identifier of the owner group
     */
    public final SIDObject getOwnerGroupSid() {
        return this.ownerGroupSid;
    }

    /**
     * Gets the owner user SIDObject of this security descriptor.
     *
     * @return the security identifier of the owner user
     */
    public final SIDObject getOwnerUserSid() {
    ...
    public int decode(final byte[] buffer, int bufferIndex, final int len) throws SMBProtocolDecodingException {
        ...
        if (ownerUOffset > 0) {
            ...
            this.ownerUserSid = new SIDObject(buffer, bufferIndex);
            bufferIndex += 8 + 4 * this.ownerUserSid.sub_authority_count;
        }

        if (ownerGOffset > 0) {
            bufferIndex = start + ownerGOffset;
            this.ownerGroupSid = new SIDObject(buffer, bufferIndex);
