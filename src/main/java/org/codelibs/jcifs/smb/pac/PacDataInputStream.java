...
public class PacDataInputStream {
    ...
    /**
     * Reads a 32-bit RID and constructs a SIDObject from it.
     * @return the constructed SIDObject object
     * @throws IOException if an I/O error occurs
     * @throws PACDecodingException if the SIDObject data is invalid
     */
    public SIDObject readId() throws IOException, PACDecodingException {
        ...
        return new SIDObject(sidBytes, 0);
    }

    /**
     * Reads a full Security Identifier (SIDObject) structure.
     * @return the SIDObject object
     * @throws IOException if an I/O error occurs
     * @throws PACDecodingException if the SIDObject data is invalid
     */
    public SIDObject readSid() throws IOException, PACDecodingException {
        final int sidSize = readInt();
        final byte[] bytes = new byte[8 + sidSize * 4];
        readFully(bytes);
        return new SIDObject(bytes, 0);
    }
