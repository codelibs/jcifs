...
public class PacLogonInfo {
    ...
    private SIDObject userSid;
    private SIDObject groupSid;
    private SIDObject[] groupSids;
    private SIDObject[] resourceGroupSids;
    private SIDObject[] extraSids;
    ...
    public PacLogonInfo(final byte[] data) throws PACDecodingException {
        try {
            ...
            // IDs for user
            final SIDObject userId = pacStream.readId();
            final SIDObject groupId = pacStream.readId();
            ...
            if (groupPointer != 0) {
                ...
                for (int i = 0; i < groupCount; i++) {
                    pacStream.align(4);
                    final SIDObject id = pacStream.readId();
            ...
            if (hasResourceGroups && resourceGroupPointer != 0) {
                ...
                for (int i = 0; i < resourceGroupCount; i++) {
                    final SIDObject id = pacStream.readSid();
                    final int attributes = pacStream.readInt();
            ...
            // Extract Extra SIDs
            this.extraSids = new SIDObject[extraSidAtts.length];
            ...
            this.resourceGroupSids = new SIDObject[resourceGroups.length];
            for (int i = 0; i < resourceGroups.length; i++) {
                this.resourceGroupSids[i] = new SIDObject(resourceDomainId, resourceGroups[i].getId());
            }

            // Compute User IDs with Domain ID to get User SIDs
            // First extra is user if userId is empty
            if (!userId.isEmpty() && !userId.isBlank()) {
                this.userSid = new SIDObject(domainId, userId);
            } else if (this.extraSids.length > 0) {
                this.userSid = this.extraSids[0];
            }
            this.groupSid = new SIDObject(domainId, groupId);

            // Compute Group IDs with Domain ID to get Group SIDs
            this.groupSids = new SIDObject[groups.length];
            for (int i = 0; i < groups.length; i++) {
                this.groupSids[i] = new SIDObject(domainId, groups[i].getId());
            }
    ...
    /**
     * Returns the user's Security Identifier (SIDObject).
     * @return the user SIDObject
     */
    public SIDObject getUserSid() {
        return this.userSid;
    }

    /**
     * Returns the user's primary group SIDObject.
     * @return the primary group SIDObject
     */
    public SIDObject getGroupSid() {
     ...
     */
    public SIDObject[] getResourceGroupSids() {
