...
import org.codelibs.jcifs.smb.internal.smb2.lease.LeaseManager;
...
public class SmbFile extends URLConnection implements SmbResource, SmbConstants {
    ...
    public boolean exists() throws SmbException {
        ...
        try {
            if (this.url.getHost().length() == 0) {
                ...
                // For share-level resources, catch tree connection errors that indicate invalid shares
                SmbTreeHandleImpl th = null;
                try {
                    th = ensureTreeConnected();
                } catch (CIFSException ce) {
                    if (this.fileLocator.getType() == TYPE_SHARE) {
                        // For shares, connection errors typically mean the share doesn't exist
                        // Check the specific error to determine if it's a non-existent share
                        if (ce instanceof SmbException) {
                            SmbException se = (SmbException) ce;
                            int status = se.getNtStatus();
                            if (status == NtStatus.NT_STATUS_BAD_NETWORK_NAME || status == NtStatus.NT_STATUS_OBJECT_NAME_NOT_FOUND
                                    || status == NtStatus.NT_STATUS_OBJECT_PATH_NOT_FOUND || status == NtStatus.NT_STATUS_BAD_DEVICE_TYPE
                        ...
                        throw SmbException.wrap(ce);
                    }
                    // For other resource types, re-throw as normal
                    throw SmbException.wrap(ce);
                }

                try (SmbTreeHandleImpl treeHandle = th) {
                    if (this.fileLocator.getType() == TYPE_SHARE) {
                        // For share root, we need to actually test access to verify existence
                        // Tree connection alone may succeed even for non-existent shares
                        try {
                            if (treeHandle.isSMB2()) {
                                // Try to query share information with minimal access
                                queryPath(treeHandle, "\\", FileInformation.FILE_BASIC_INFO);
                            } else {
                                // For SMB1, try to list directory
                                queryPath(treeHandle, "\\", FileInformation.FILE_BASIC_INFO);
                            }
                        } catch (SmbException se) {
                            // Debug logging to understand what error we get
                            log.debug("Share access error for {}: NtStatus=0x{}, message={}", this.fileLocator.getUNCPath(),
                                    Integer.toHexString(se.getNtStatus()), se.getMessage());

                            // Re-throw specific share access errors
                            switch (se.getNtStatus()) {
                            case NtStatus.NT_STATUS_BAD_NETWORK_NAME:
                            case NtStatus.NT_STATUS_BAD_DEVICE_TYPE:
                            ...
                            case NtStatus.NT_STATUS_SHARING_VIOLATION:
                                throw se; // Share doesn't exist or not accessible
                            default:
                                // Other errors still indicate share exists but has issues
                                log.debug("Non-fatal share access error, assuming share exists: 0x{}",
                                        Integer.toHexString(se.getNtStatus()));
                                break;
                            }
                        }
                        log.trace("Share root existence verified through directory query");
                    } else {
                        ...
                        if (treeHandle.isSMB2()) {
                            try {
                                ...
                                withOpen(treeHandle, Smb2CreateRequest.FILE_OPEN, SmbConstants.FILE_READ_ATTRIBUTES,
                            ...
                            queryPath(treeHandle, this.fileLocator.getUNCPath(), FileInformation.FILE_BASIC_INFO);
                        }
                    }
                }
            }
            ...
            // For share-level resources, certain errors should be thrown per SMB spec
            try {
                if (this.fileLocator.getType() == TYPE_SHARE) {
                    switch (se.getNtStatus()) {
                    case NtStatus.NT_STATUS_BAD_NETWORK_NAME:
                    ...
                    case NtStatus.NT_STATUS_OBJECT_NAME_NOT_FOUND:
                    case NtStatus.NT_STATUS_OBJECT_PATH_NOT_FOUND:
                        // Invalid or inaccessible share should throw exception
                        throw se;
                    }
                }
            } catch (CIFSException ce) {
    ...
    void copyRecursive(final SmbFile dest, final byte[][] b, final int bsize, final WriterThread w, final SmbTreeHandleImpl sh,
        ...
        try {
            ...
            // Re-throw SmbException as CIFSException while preserving original cause
            throw new CIFSException("Copy operation failed", e);
        } catch (Exception e) {
            // Handle unexpected errors during copy
            throw new CIFSException("Unexpected error during copy", e);
        }
    }
    ...
    void setPathInformation(final int attrs, final long ctime, final long mtime, final long atime) throws CIFSException {
        try (SmbTreeHandleImpl th = ensureTreeConnected()) {
            ...
            if (th.isSMB2()) {
                ...
                withOpen(th, Smb2CreateRequest.FILE_OPEN, FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                        req);
            } else if (th.hasCapability(SmbConstants.CAP_NT_SMBS)) {

                try (SmbFileHandleImpl f = openUnshared(O_RDONLY, FILE_WRITE_ATTRIBUTES,
                        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, dir, dir != 0 ? 0x0001 : 0x0040)) {
    ...
    public void setAttributes(final int attrs) throws SmbException {
        ...
        // Clear any cached handles and attributes to avoid sharing violations
        clearAttributeCache();
        close();
    ...
    private void processAces(final ACE[] aces, final boolean resolveSids) throws IOException {
        ...
        if (resolveSids) {
            ...
            final SIDObject[] sids = new SIDObject[aces.length];
    ...
    @Override
    public SIDObject getOwnerUser() throws IOException {
        return getOwnerUser(true);
    }

    @Override
    public SIDObject getOwnerUser(final boolean resolve) throws IOException {
        try (SmbTreeHandleImpl th = ensureTreeConnected()) {
            final SecurityDescriptor desc = querySecurity(th, SecurityInfo.OWNER_SECURITY_INFO);
            final SIDObject ownerUser = desc.getOwnerUserSid();
            if (ownerUser == null) {
                return null;
            }
            ...
            if (resolve) {
                ...
                try {
                    ...
                    log.warn("Failed to resolve SIDObject " + ownerUser.toString(), e);
                }
            } else {
    ...
    @Override
    public SIDObject getOwnerGroup() throws IOException {
        return getOwnerGroup(true);
    }

    @Override
    public SIDObject getOwnerGroup(final boolean resolve) throws IOException {
        try (SmbTreeHandleImpl th = ensureTreeConnected()) {
            final SecurityDescriptor desc = querySecurity(th, SecurityInfo.GROUP_SECURITY_INFO);
            final SIDObject ownerGroup = desc.getOwnerGroupSid();
            if (ownerGroup == null) {
                return null;
            }
            ...
            if (resolve) {
                ...
                try {
                    ...
                    log.warn("Failed to resolve SIDObject " + ownerGroup.toString(), e);
                }
            } else {
