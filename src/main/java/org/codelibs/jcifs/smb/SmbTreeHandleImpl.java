...
class SmbTreeHandleImpl implements SmbTreeHandleInternal {
    ...
    public boolean isSMB3() {
        try (SmbSessionImpl session = this.treeConnection.getSession(); SmbTransportImpl transport = session.getTransport()) {
            return transport.isSMB2()
                    && transport.getNegotiateResponse().getSelectedDialect().atLeast(org.codelibs.jcifs.smb.DialectVersion.SMB300);
        } catch (final SmbException e) {
    ...
    public boolean isSMB30() {
        try (SmbSessionImpl session = this.treeConnection.getSession(); SmbTransportImpl transport = session.getTransport()) {
            org.codelibs.jcifs.smb.DialectVersion dialect = transport.getNegotiateResponse().getSelectedDialect();
            return transport.isSMB2() && dialect.atLeast(org.codelibs.jcifs.smb.DialectVersion.SMB300)
                    && !dialect.atLeast(org.codelibs.jcifs.smb.DialectVersion.SMB311);
        } catch (final SmbException e) {
