...
public class SmbFileOutputStream extends OutputStream {
     ...
     * an SMB server represented by the {@link org.codelibs.jcifs.smb.SmbFile} parameter. See
     * {@link org.codelibs.jcifs.smb.SmbFile} for a detailed description and examples of
     * the smb URL syntax.
     *
     * @param file
    ...
    SmbFileOutputStream(final SmbFile file, final SmbTreeHandleImpl th, final SmbFileHandleImpl handle, final int openFlags,
            final int access, final int sharing) throws CIFSException {
        this.file = file;
        this.handle = handle;
        this.openFlags = openFlags;
        this.access = access;
    ...
    /**
     * Flushes this output stream and forces any buffered output bytes to be written out to the server.
     * This ensures data persistence according to SMB protocol specifications.
     *
    ...
    public void flush() throws IOException {
        if (this.handle != null && this.handle.isValid()) {
            try {
                ...
                if (this.smb2 && th != null) {
                    // For SMB2/3, send a flush request to ensure data is persisted
                    Smb2FlushRequest flushReq = new Smb2FlushRequest(th.getConfig(), this.handle.getFileId());
                    th.send(flushReq, RequestParam.NO_RETRY);
                }
                // For SMB1, writes are synchronous by default

                // Clear file attributes cache so length() returns updated value
