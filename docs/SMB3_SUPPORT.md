...
| `jcifs.client.followSymlinks` | `false` | Resolve a symbolic link the server declines to follow, and reissue the request against its target — see [Other features](#other-features). Only a relative target is followed; an absolute one is reported as before. |
